...
| ได้รับงานผ่านช่องทางที่ระบุผู้ส่งหรือผู้รับผล | ก่อนจบหรือคืนการควบคุม ให้ส่ง report ที่มีหลักฐานผ่าน return/coordination channel ที่ผู้รับเข้าถึงได้และตรวจว่า delivery สำเร็จ; ใน teammate/subagent session ให้ใช้ `SendMessage` เสมอ รวมถึงกรณี “ไม่มีข้อมูลใหม่” หรือ “ส่งครบแล้ว”; final prose ใน local session หรือสถานะ idle ไม่ใช่หลักฐานว่าส่งมอบแล้ว |
| ได้รับ `idle_notification` จาก teammate/subagent | ถือเป็นเพียงสถานะว่าง/พร้อมรับงาน ไม่ใช่ report หรือหลักฐานว่าไม่มีผลลัพธ์; ตรวจข้อความตอบกลับและ delivery ของงานล่าสุดก่อนสรุปหรือแจ้งผู้ใช้ |
...
| งานมี primary artifact/outcome และต้องผ่าน prerequisite, probe หรือ diagnostic หลายขั้น | anchor task แรกด้วย `primary deliverable + acceptance evidence`; map งานเสริมเป็น child, รายงาน deliverable progress แยกจาก readiness และ resume parent เมื่อ child จบ. ห้ามนับ child ที่จบเป็นความคืบหน้าของ deliverable หรือแทน deliverable ด้วย finding/report |
...
| ผู้ใช้อนุมัติ prerequisite, probe หรือ diagnosis ที่รับใช้ current objective | ถือเป็น child objective โดย default; จำกัด investigation เท่าที่ทำให้ parent เดินต่อได้ แล้ว resume primary deliverable. การอนุมัติวิธีทำไม่ใช่การเปลี่ยนรูปแบบผลส่งมอบ |
...
| จะเลือก test level/matrix/fixture, สร้าง load-test/benchmark harness, แก้ regression, เปลี่ยน logic/boundary ที่มีความเสี่ยง, suite ผ่านแต่ flow ยังพัง หรือ coverage gap ไม่ชัด | invoke `testing-strategy`; ผูก claim กับ failure mode และ observable evidence. การรัน verification command ที่ repo/criterion กำหนดชัดแล้วไม่ต้อง invoke; route ไป `performance` เพิ่มเมื่อจะวิเคราะห์ metric หรือเลือก optimization |
| จะวางแผน ออกแบบ review สร้าง แก้ หรือ delegate งาน frontend, UI, user-facing state หรือ product copy | invoke `ui-ux-baseline` และ child ที่ตรงก่อนออกแบบหรือเขียนโค้ด; prompt ที่ส่งต่อไม่ทดแทน skill routing และผู้รับงานต้องใช้มาตรฐานเดียวกันกับ agent หลัก |
