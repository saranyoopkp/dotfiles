...
| กำลังปิดงาน mutation ที่อนุมัติ หรือพบ quirk/pain/preference ที่ควรอยู่ข้าม session | ตรวจ CLAUDE.md/docs/memory ทันที; จดเฉพาะสิ่งที่โค้ดเล่าเองไม่ได้, quirk ใช้ `symptom → root cause → fix`. หากผู้ใช้ทักเรื่องเดิมครั้งที่สองให้เสนอ durable record และอัปเดตเมื่ออยู่ใน scope |
