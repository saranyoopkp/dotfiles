...
| จะ delegate mutation ผ่าน isolated subagent | ใช้เฉพาะ slice ที่เริ่มจาก canonical base ได้ อิสระ และ review/rollback แยกได้. ยืนยัน Git root, expected starting revision และ PR base แล้วเรียก `Agent` ด้วย `isolation: "worktree"`; runtime เป็นผู้สร้างและ assign worktree. ถ้า revision ผิดให้หยุดและ spawn ใหม่ ห้ามให้ worker `EnterWorktree`, สร้าง worktree ซ้อน หรือย้าย checkout |
...
| หลาย workers ต้องใช้ local foundation เดียวกัน | coordinator เข้า integration worktree, ทำและ commit foundation ก่อน spawn Agent Team. Teammates ใช้ lead CWD เดียวกันโดยไม่มี per-teammate worktree; แบ่ง owned paths ไม่ให้ทับกัน และ coordinator เป็น owner ของ Git index, shared files, commit และ PR เดียว. Formatter/codegen หรือ runtime resource ที่แชร์ต้องมี owner/namespace หรือลำดับชัด |
...
| coordinator ได้ report จากผู้รับงาน | ตรวจ starting revision, material claim, diff และ artifact ปัจจุบันก่อน integrate; report เป็น input ไม่ใช่ acceptance evidence โดยอัตโนมัติ. Isolated subagent ส่ง branch/commit ผ่าน PR เข้า declared base—ห้าม direct merge/cherry-pick เข้า root checkout หรือ `main`. Shared team worktree ให้ coordinator commit และเปิด PR เดียว. Push, เปิด PR และ merge ต้องอยู่ใน authorization ของ workflow นั้น |
