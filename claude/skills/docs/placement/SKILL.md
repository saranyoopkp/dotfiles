...
| **inline comment** | ตาแตะบรรทัดนั้นตอนแก้ | constraint/why ที่โค้ดแสดงเองไม่ได้ + guard ณ จุดแก้ ("ห้ามแก้โดยไม่ X") | **≤2 บรรทัด + pointer** |
...
- **≤2 บรรทัด**: การตัดสินใจ/ข้อจำกัด + pointer — รายละเอียด/ประวัติ/ผลทดลอง → docs/ เสมอ
  ```python
...
- **ก่อนกดเขียน comment เกิน 2 บรรทัด = สัญญาณว่ากำลังเขียน docs ผิดที่** — หยุด ย้าย
...
แก้จุดนั้นต้องเห็น เก็บไว้ ≤2 บรรทัด; บั๊กเก่า/ผลวัด/ประวัติเวอร์ชัน → docs + pointer —
...
ไล่ต่อไฟล์:
1. **comment ยาว (>2 บรรทัด)** — จำแนกเนื้อทีละก้อน:
   - why/constraint จริง → บีบเหลือ ≤2 บรรทัด + pointer
