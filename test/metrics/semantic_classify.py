...
Do not edit this file while a batch is running; Bash reads scripts incrementally.
"""
...
BATCH = 80
PARALLEL = 8
USE_CTX = False  # See the evaluation warning above PROMPT_HEAD.

# Do not change this prompt without rerunning eval_groundtruth; it is a stable measurement tool.
# Sweep 2026-07-17 against GT n=100: baseline Haiku without context scored 0.84, the best result.
# Haiku+context 0.76; Haiku+context+correction rules 0.59; Sonnet 0.69; Sonnet+context 0.70.
# Raw-case review found that the 150-character tail of the previous assistant message was often
# stale, interrupted by tool calls, or cut mid-sentence. Irrelevant context shifted 13 of 16
# degraded cases to new_task. This does not prove that high-quality context is harmful.
# Most Sonnet misses were taxonomy disagreements on terse commands, not comprehension failures;
# accuracy measures agreement with Fable-judge labels, not absolute truth. Context is off by default.
# Adding user_takeover as an eighth category reduced overall accuracy from 0.84 to 0.69, so takeover
# uses two passes: an eight-category candidate pass followed by judge confirmation. On 07-17 this
# produced 55 candidates, 13 confirmed trust-loss cases, and 7 from the hook-saga day.
PROMPT_HEAD = """Classify each user message from a coding-assistant conversation into exactly one category:
- corr_target: user corrects WRONG TARGET/intent (assistant misread what/where, e.g. "not that one", "I meant...", wrong repo/file/scope)
- corr_omit: user points out something MISSED/forgotten/incomplete
- corr_over: user says assistant did/proposed TOO MUCH or unnecessary ("don't do that", revert extra work)
- approve_continue: user merely approves/tells assistant to continue its own stated plan ("continue", "keep going", "ok", "yes, do it")
...
def main():
    ...
    ap.add_argument("--ctx", action="store_true", help="include previous-assistant context; measured worse, see comments")
